...
function proxiedPop() {
    ...
    const changes = [
        new Change(DELETE, [poppedIndex], undefined, popResult, this, utils.copy(this)),
    ];
...
function proxiedPush() {
    ...
    for (let i = initialLength, j = target.length; i < j; i++) {
        changes[i - initialLength] = new Change(INSERT, [i], target[i], undefined, this, utils.copy(this));
    }
    oMeta.callObservers(changes);
    return pushResult;
}
...
function proxiedUnshift() {
    ...
    for (let i = 0; i < l; i++) {
        changes[i] = new Change(INSERT, [i], target[i], undefined, this, utils.copy(this));
    }
    oMeta.callObservers(changes);
    ...
    if (start < tarLen && end > start) {
        ...
        for (let i = start, item, tmpTarget; i < end; i++) {
            ...
            if (i in prev) {
                ...
                changes.push(new Change(UPDATE, [i], target[i], tmpTarget, this, utils.copy(this)));
            }
            else {
                changes.push(new Change(INSERT, [i], target[i], undefined, this, utils.copy(this)));
            }
        }
        oMeta.callObservers(changes);
    }
...
function proxiedCopyWithin(dest, start, end) {
    ...
    if (dest < tarLen && dest !== start && len > 0) {
        ...
        for (let i = dest, nItem, oItem, tmpObserved; i < dest + len; i++) {
            ...
            changes.push(new Change(UPDATE, [i], nItem, oItem, this, utils.copy(this)));
        }
        oMeta.callObservers(changes);
    }
    return this;
...
function proxiedSplice() {
    ...
    for (index = 0; index < removed; index++) {
        if (index < inserted) {
            changes.push(new Change(UPDATE, [startIndex + index], target[startIndex + index], spliceResult[index], this, utils.copy(this)));
        }
        else {
            changes.push(new Change(DELETE, [startIndex + index], undefined, spliceResult[index], this, utils.copy(this)));
        }
    }
    for (; index < inserted; index++) {
        changes.push(new Change(INSERT, [startIndex + index], target[startIndex + index], undefined, this, utils.copy(this)));
    }
    oMeta.callObservers(changes);
    return spliceResult;
}
